...
class EMBLBeamlineTest(HardwareObject):
    ...
    def center_beam_test_task(self):
        ...
        log.info("Beam centering: Active mode %s" % active_mode)

        if active_mode in (None, "Imaging", "TREXX"):

            log.info("Beam centering: doing pitch scan only")
            self.do_p14_pitch_optimization()
            log.info("Beam centering: done")
            return
        ...
        with gevent.Timeout(30, Exception("Timeout waiting for diffractometer ready")):
            while not self.bl_hwobj.diffractometer_hwobj.is_ready():
                gevent.sleep(0.1)


        self.bl_hwobj.diffractometer_hwobj.set_phase(
            self.bl_hwobj.diffractometer_hwobj.PHASE_BEAM, timeout=None
        )
        self.bl_hwobj.diffractometer_hwobj.wait_device_not_ready(1)
        """
        ...
        if self.bl_hwobj.session_hwobj.beamline_name == "P13":
            ...
            if active_mode in ("Collimated", "Imaging", "TREXX"):
                transmission_value = new_transmission
                zoom_value = "Zoom 4"
            else:
                transmission_value = 2.0
                zoom_value = "Zoom 8"

            self.bl_hwobj.transmission_hwobj.set_value(transmission_value, timeout=None)
            ...
            # GB Synchronize phase settings here:
            #self.bl_hwobj.diffractometer_hwobj.wait_device_ready(30) 
            with gevent.Timeout(30, Exception("Timeout waiting for diffractometer ready")):
                while not self.bl_hwobj.diffractometer_hwobj.is_ready():
                    gevent.sleep(0.1)

            with gevent.Timeout(20, Exception("Timeout waiting for phase %s" %  self.bl_hwobj.diffractometer_hwobj.PHASE_BEAM)):
                while  self.bl_hwobj.diffractometer_hwobj.PHASE_BEAM !=  self.bl_hwobj.diffractometer_hwobj.chan_current_phase.getValue():
                    gevent.sleep(0.1)
            ...
            self.bl_hwobj.diffractometer_hwobj.set_zoom(zoom_value)
    ...
    def center_beam_task(self):
        ...
        if self.bl_hwobj.session_hwobj.beamline_name == "P13":
            ...
            if True:
                ...
                delta_hor = 1.0 
                for i in range(5):
                    if abs(delta_hor) < 0.001 and abs(delta_ver) < 0.0003:
                       break
    ...
    def do_p14_pitch_optimization(self):
        self.cmd_set_pitch_position(0)
        self.cmd_set_pitch(1)

        offset = tine.get("/p14/P14MonoMotor/Pitch2nd","PositionOffset", 5000)
        scale = tine.get("/p14/P14MonoMotor/Pitch2nd","PositionScale", 5000)
        ...
        if self.bl_hwobj._get_energy() < 10:
           tine.set("/p14/p14mono.cdi/setUndGap","SEND",und)
           self.crl_hwobj.set_crl_value(crl_value, timeout=30)
          
        enc  = tine.get("/p14/p14mono.cdi/EncAr","RECV",5000,750,1)
        ptch = tine.get("/p14/p14mono.cdi/VPitchAr","RECV",5000,750,1)
        flux = numpy.array(tine.get("/p14/p14mono.cdi/QBPMAr","RECV",5000,750,1))
        imax = numpy.where(flux==max(flux))[0][0]
        enc_target = enc[imax] * scale + offset
        y=ptch[imax]
        ...
        for n in range(9):
           ...
           self.cmd_set_pitch(1)
           sleep(1)
           x =  tine.get("/p14/p14monomotor/Pitch2nd","position",5000,1)
           dx = x - enc_target
           y = y - 0.1 * dx
           logging.getLogger("HWR").info("Histeresis correction Step: %s Encoder: %s Voltage: %s Shift: %s"%(n,x,y,dx))
           if abs(dx) < 0.25:
              break
